...
public class HotelMoreTests {
    ...
    void statsPopularRoomsOrder() {
        ...
        List<Room> popular = new com.example.hotel.web.StatsController(new com.example.hotel.repo.RoomRepository() {
            @Override public List<Room> findAll() { return List.of(r1, r2); }
            @Override public List<Room> findAllById(Iterable<Long> longs) { return null; }
            @Override public <S extends Room> List<S> saveAll(Iterable<S> entities) { return null; }
            ...
            @Override public <S extends Room> List<S> insert(Iterable<S> entities) { return null; }
            @Override public <S extends Room> S saveAndFlush(S entity) { return null; }
            @Override public <S extends Room> List<S> saveAllAndFlush(Iterable<S> entities) { return null; }
            ...
            @Override public <S extends Room> List<S> findAll(org.springframework.data.domain.Example<S> example) { return null; }
            @Override public <S extends Room> List<S> findAll(org.springframework.data.domain.Example<S> example, org.springframework.data.domain.Sort sort) { return null; }
            ...
            @Override public List<Room> findAll(org.springframework.data.domain.Sort sort) { return List.of(r1, r2).stream().sorted(sort).toList(); }
